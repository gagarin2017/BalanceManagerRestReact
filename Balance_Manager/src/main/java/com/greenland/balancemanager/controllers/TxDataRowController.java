...
public class TxDataRowController {
	...
	public List<TxDataRow> getAllTransactionRows() {
		...
		final TxDataRow txDataRow = TxDataRow.builder().uuid(UUID.randomUUID()).txDate(LocalDate.now())
				.accountName("Account Name").categoryName("Categopry name").created_At(LocalDate.now())
				.creditAmount(new BigDecimal("0")).debitAmount(new BigDecimal("123.01")).isReconsiled(false)
				.isRemote(true).build();
